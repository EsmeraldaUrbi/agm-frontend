...
export class ApiClient {
  ...
  get<T>(url: string, params?: HttpParams | { [param: string]: string | number | boolean | ReadonlyArray<string | number | boolean> }): Observable<T> {
    return this.http.get<T>(normalizeApiUrl(url), { params }).pipe(
      catchError(this.handleError)
    );
  }

  post<T>(url: string, body: any, options?: { headers?: HttpHeaders; params?: HttpParams }): Observable<T> {
    return this.http.post<T>(normalizeApiUrl(url), body, options).pipe(
      catchError(this.handleError)
    );
  }

  put<T>(url: string, body: any): Observable<T> {
    return this.http.put<T>(normalizeApiUrl(url), body).pipe(
      catchError(this.handleError)
    );
  }

  patch<T>(url: string, body: any): Observable<T> {
    return this.http.patch<T>(normalizeApiUrl(url), body).pipe(
      catchError(this.handleError)
    );
  }

  delete<T>(url: string): Observable<T> {
    return this.http.delete<T>(normalizeApiUrl(url)).pipe(
      catchError(this.handleError)
    );
  }
  ...
  download(url: string, params?: HttpParams | { [param: string]: string | number | boolean }): Observable<Blob> {
    ...
    return this.http.get(normalizeApiUrl(url), {
      headers,
