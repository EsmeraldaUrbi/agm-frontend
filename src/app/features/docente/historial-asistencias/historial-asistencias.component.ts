...
export class HistorialAsistenciasComponent implements OnInit {
  ...
  resolverDocenteYCargarCursos() {
    ...
    if (!user?.email) {
      ...
      this.mensajeInfo.set('No se pudo identificar el correo del usuario autenticado.');
      return;
    }
    ...
    this.docentesService.getDocentes({ limit: 500 }).subscribe({
      next: (docentes) => {
        ...
        this.mensajeInfo.set('No se encontró un docente asociado al correo del usuario autenticado.');
        this.isLoading.set(false);
      ...
      error: (err) => {
        ...
        this.listaMateriasDisponibles = [];
        this.historialAlumnos.set([]);
        this.mensajeInfo.set('No se pudo resolver el docente asociado al usuario autenticado.');
        this.isLoading.set(false);
      }
    });
  }
